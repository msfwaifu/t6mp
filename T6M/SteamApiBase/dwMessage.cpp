...
void dwMessage::send(bool encrypted)
{
	...
	else
	{
		...
		*(int*)(tempBytes + 5) = 0x13371337;
		...
		dw_Crypto::dw_calculate_iv(0x13371337, iv);
		dw_Crypto::dw_get_global_key(key);
		dw_Crypto::dw_encrypt_data((char*)_bytes, iv, key, (char*)(tempBytes + 9), length);
	}
	dw_Handler::dw_queue_packet((char*)tempBytes, tempLength);
	delete[] tempBytes;
}