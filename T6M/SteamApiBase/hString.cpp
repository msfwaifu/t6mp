...
const char *hString::va(const char *fmt, ...)
{
	...
	{
		Log::Error("", "%s - Attempted to overrun string, increase the buffer. StrLen: %i", __FUNCTION__, StringLength);
	}
	ThreadSafe.unlock();
...
std::string hString::ToReadableString(std::basic_string<uint8_t> &Input, const char *Token)
{
	...
	for (uint32_t i = 0; i < Input.size(); i++)
	{
		Result.append(hString::va("%02X%s", *(uint8_t *)(Input.data() + i), Token));
	}
